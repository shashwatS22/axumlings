// Use ? to propagate errors from a handler; return Result<impl IntoResponse, AppError>.
//
// Hint: fn handler() -> Result<impl IntoResponse, AppError> and use ? on fallible calls.
...
async fn handler(Path(id): Path<u32>) -> Result<String, NotFound> {
    // TODO: Call find_item(id)? and return Ok(name)
    let _ = id;
