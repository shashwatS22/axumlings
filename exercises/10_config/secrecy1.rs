...
#[derive(Clone)]
struct AppState {
    // TODO: Change to api_key: Secret<String>
    api_key: String,
}
...
async fn handler(State(state): State<Arc<AppState>>, headers: axum::http::HeaderMap) -> impl IntoResponse {
    let key = headers.get("x-api-key").and_then(|v| v.to_str().ok());
    // TODO: Compare key with state.api_key.expose_secret() when api_key is Secret<String>
    if key != Some(&state.api_key) {
