// Use a transaction: POST /transfer moves amount from one account to another atomically.
//
// Hint: pool.begin().await, tx.execute(...).await, tx.commit().await
...
async fn handler(
    ...
    // TODO: Begin transaction, UPDATE accounts SET balance = balance - ? WHERE id = ?,
    // UPDATE accounts SET balance = balance + ? WHERE id = ?, commit
    "ok"
}
